# orderregistratiecomponent chart

The Helm chart installs orderregistratiecomponent and by default the following dependencies using subcharts:
...
```bash
helm repo add orderregistratiecomponent https://raw.githubusercontent.com/ConductionNL/orderregistratiecomponent/master/api/helm/
helm repo update
```
...
```bash
helm install my-orderregistratiecomponent ConductionNL/orderregistratiecomponent --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `orderregistratiecomponent` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `orderregistratiecomponent` |
